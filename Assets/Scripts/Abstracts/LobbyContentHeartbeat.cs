...
public class LobbyContentHeartbeat : IReceiveMessages
{
    ...
    public void OnReceiveMessage(MessageType type, object msg)
    {
        if (type == MessageType.ClientUserSeekingDisapproval)
        {
            ...
            if (shouldDisapprove)
                (msg as Action<LobbyRelaySample.relay.Approval>)?.Invoke(LobbyRelaySample.relay.Approval.GameAlreadyStarted);
        }
    }
    ...
    private void OnUpdate(float dt)
    {
        ...
        void OnRetrieve()
        {
			...
			LobbyRelaySample.lobby.ToLocalLobby.Convert(lobbyRemote, m_localLobby);
